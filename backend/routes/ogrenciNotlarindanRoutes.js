...
router.get('/', async (req, res) => {
    try {
        ...
        console.log("GET /api/ogrencinotlarindan - Gelen userId:", userId);
        ...
        if (userId) {
            ...
            const [rows] = await db.query('SELECT * FROM ogrenci_not WHERE ogrenci_id IN (?)', [ogrenciIds]);
            console.log("GET /api/ogrencinotlarindan - userId ile filtreli bildirimler getirildi:", rows.length);
            console.log(ogrenciIds)
            return res.json(rows);
        } else {
            // userId yoksa tüm bildirimleri getir
            const [rows] = await db.query('SELECT * FROM ogrenci_not');
            
            console.log("GET /api/ogrencinotlarindan - getirildi:", rows.length);
            return res.json(rows);
        }
    } catch (error) {
