...
router.get('/', async (req, res) => {
    try {
        ...
        console.log("GET /api/ogrencidevamsizliklari - Gelen userId:", userId);
        ...
        if (userId) {
            ...
            const [rows] = await db.query('SELECT * FROM devamsizlik WHERE ogrenci_id IN (?)', [ogrenciIds]);
            console.log("GET /api/ogrencidevamsizliklari - userId ile filtreli bildirimler getirildi:", rows.length);
            console.log(ogrenciIds)
            return res.json(rows);
        } else {
            // userId yoksa tüm bildirimleri getir
            const [rows] = await db.query('SELECT * FROM devamsizlik');
            
            console.log("GET /api/ogrencidevamsizliklari - getirildi:", rows.length);
            return res.json(rows);
        }
    } catch (error) {
