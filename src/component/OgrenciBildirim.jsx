...
const API_URL = 'http://localhost:5000/api/ogrencibildirim';
                    ...
                    isMobile ? (
                        ...
                        <Grid container spacing={2}>
                            {ogrenciler.length === 0 ? (
                                ...
                                ogrenciler.map((bildirim) => (
                                    <Grid item xs={12} key={bildirim.id}>
                                        <Card variant="outlined" className="ogrenci-kart">
                                            <CardContent>
                                                <Typography variant="h6" gutterBottom>{bildirim.id}</Typography>
                                                <Typography color="textSecondary">Numara: {bildirim.bildirim_icerigi}</Typography>
                                                <Typography color="textSecondary">Sınıf: {bildirim.bildirim_turu_id || '-'}</Typography>
                                                <Typography color="textSecondary">Veli ID: {bildirim.created_at || '-'}</Typography>
                                            </CardContent>
                                ...
                                <Table stickyHeader className="ogrenci-tablosu" aria-label="Öğrenci Listesi Tablosu">
                                    <TableHead>
                                        <TableRow>
                                            ...
                                            <TableCell>Bildirim içeriği</TableCell>
                                            <TableCell>Bildirim Türü</TableCell>
                                            <TableCell>Öğrenci id </TableCell>
                                            <TableCell>Gelme Zamanı</TableCell>
                                        ...
                                        {ogrenciler.length === 0 ? (
                                            ...
                                            ogrenciler.map((bildirim) => (
                                                <TableRow key={bildirim.id} hover>
                                                    <TableCell>{bildirim.id}</TableCell>
                                                    <TableCell>{bildirim.bildirim_icerigi}</TableCell>
                                                    <TableCell>{bildirim.bildirim_turu_id}</TableCell>
                                                    <TableCell>{bildirim.ogrenci_id}</TableCell>
                                                    <TableCell>{bildirim.created_at}</TableCell>
                                                    <TableCell align="right">
                                                        <IconButton
                                                            size="small"
                                                            onClick={() => handleOpenDialog(bildirim)}
                                                            className="edit-icon"
                                                            aria-label={`Düzenle bildirim ${bildirim.id}`}
                                                            sx={{ mr: 1 }}
                                                        >
                                                        ...
                                                        <IconButton
                                                            size="small"
                                                            onClick={() => handleOpenDeleteConfirm(bildirim)}
                                                            className="delete-icon"
                                                            aria-label={`Sil bildirim ${bildirim.id}`}
                                                        >
