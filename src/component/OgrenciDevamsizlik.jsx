...
const API_URL = 'http://localhost:5000/api/ogrencidevamsizliklari';
...
function OgrenciIslemleri() {
    ...
    const fetchOgrenciler = useCallback(async () => {
        ...
        try {
            ...
            const currentUserId = localStorage.getItem('userId'); // Kullanıcı ID'sini localStorage'dan al
            console.log("Kullanıcı ID:", currentUserId);
            
            // userId varsa query parametresi olarak gönder
            let url = API_URL;
            if (currentUserId) {
                    ...
                    isMobile ? (
                        ...
                        <Grid container spacing={2}>
                            {ogrenciler.length === 0 ? (
                                ...
                                ogrenciler.map((devamsizlik) => (
                                    <Grid item xs={12} key={devamsizlik.id}>
                                        <Card variant="outlined" className="ogrenci-kart">
                                            <CardContent>
                                                <Typography variant="h6" gutterBottom>{devamsizlik.id}</Typography>
                                                <Typography color="textSecondary">Numara: {devamsizlik.ogrenci_id}</Typography>
                                                <Typography color="textSecondary">Sınıf: {devamsizlik.devamsizlik_tarihi || '-'}</Typography>
                                                <Typography color="textSecondary">Veli ID: {devamsizlik.devamsizlik_turu || '-'}</Typography>
                                                <Typography color="textSecondary">Veli ID: {devamsizlik.aciklama || '-'}</Typography>
                                                <Typography color="textSecondary">Veli ID: {devamsizlik.created_at || '-'}</Typography>
                                            </CardContent>
                                ...
                                <Table stickyHeader className="ogrenci-tablosu" aria-label="Öğrenci Listesi Tablosu">
                                    <TableHead>
                                        <TableRow>
                                            ...
                                            <TableCell>ogrenci_id</TableCell>
                                            <TableCell>devamsizlik_tarihi</TableCell>
                                            <TableCell>devamsizlik_turu</TableCell>
                                    ...
                                    <TableBody>
                                        {ogrenciler.length === 0 ? (
                                            ...
                                            ogrenciler.map((devamsizlik) => (
                                                <TableRow key={devamsizlik.id} hover>
                                                    <TableCell>{devamsizlik.id}</TableCell>
                                                    <TableCell>{devamsizlik.ogrenci_id}</TableCell>
                                                    <TableCell>{devamsizlik.devamsizlik_tarihi}</TableCell>
                                                    <TableCell>{devamsizlik.devamsizlik_turu}</TableCell>
                                                    <TableCell>{devamsizlik.aciklama}</TableCell>
                                                    <TableCell>{devamsizlik.created_at}</TableCell>
                                                    <TableCell align="right">
                                                        <IconButton
                                                            size="small"
                                                            onClick={() => handleOpenDialog(devamsizlik)}
                                                            className="edit-icon"
                                                            aria-label={`Düzenle bildirim ${devamsizlik.id}`}
                                                            sx={{ mr: 1 }}
                                                        >
                                                        ...
                                                        <IconButton
                                                            size="small"
                                                            onClick={() => handleOpenDeleteConfirm(devamsizlik)}
                                                            className="delete-icon"
                                                            aria-label={`Sil bildirim ${devamsizlik.id}`}
                                                        >
